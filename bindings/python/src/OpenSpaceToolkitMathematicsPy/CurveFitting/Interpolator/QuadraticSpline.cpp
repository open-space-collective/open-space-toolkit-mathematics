...
inline void OpenSpaceToolkitMathematicsPy_CurveFitting_Interpolator_QuadraticSpline(pybind11::module& aModule)
{
    ...
    class_<QuadraticSpline, Interpolator, Shared<QuadraticSpline>>(aModule, "QuadraticSpline")

        .def(
            init<const VectorXd&, const VectorXd&>(),
            R"doc(
                Create a quadratic spline interpolator with data points.
                ...
                Example:
                    ...
                    >>> interpolator = QuadraticSpline(x, y)
            )doc",
            arg("x"),
            arg("y")
        )
        .def(
            ...
            R"doc(
                Create a quadratic spline interpolator with uniform spacing.
                ...
                Example:
                    >>> y = np.array([2.0, 0.0, 0.0, 2.0])
                    >>> interpolator = QuadraticSpline(y, 0.0, 1.0)  # x = [0, 1, 2, 3]
            )doc",
            arg("y"),
            arg("x_0"),
            arg("h")
        ...
        .def(
            "evaluate",
            overload_cast<const VectorXd&>(&QuadraticSpline::evaluate, const_),
            R"doc(
                Evaluate the quadratic spline interpolation at multiple points.
                ...
                Example:
                    >>> interpolator = QuadraticSpline([2.0, 0.0, 0.0, 2.0], 0.0, 1.0)
        ...
        .def(
            "evaluate",
            overload_cast<const double&>(&QuadraticSpline::evaluate, const_),
            R"doc(
                Evaluate the quadratic spline interpolation at a single point.
        ...
        .def(
            "compute_derivative",
            overload_cast<const double&>(&QuadraticSpline::computeDerivative, const_),
            R"doc(
                Compute the derivative of the quadratic spline at a single point.
        ...
        .def(
            "compute_derivative",
            overload_cast<const VectorXd&>(&QuadraticSpline::computeDerivative, const_),
            R"doc(
                Compute the derivative of the quadratic spline at multiple points.
