...
RotationMatrix::RotationMatrix(
    ...
    : matrix_((Matrix3d() << aFirstCoefficient,
               aSecondCoefficient,
               aThirdCoefficient,
               aFourthCoefficient,
               aFifthCoefficient,
...
double RotationMatrix::operator()(const Index& aRowIndex, const Index& aColumnIndex) const
{
    if (aRowIndex > 3)
    {
...
double& RotationMatrix::operator()(const Index& aRowIndex, const Index& aColumnIndex)
{
    if (aRowIndex > 3)
    {
        throw ostk::core::error::RuntimeError("Row index out of bounds.");
...
std::ostream& operator<<(std::ostream& anOutputStream, const RotationMatrix& aRotationMatrix)
{
    ostk::core::utils::Print::Header(anOutputStream, "Rotation Matrix");

    ostk::core::utils::Print::Line(anOutputStream)
        << aRotationMatrix.matrix_(0, 0) << aRotationMatrix.matrix_(0, 1) << aRotationMatrix.matrix_(0, 2);
    ostk::core::utils::Print::Line(anOutputStream)
        << aRotationMatrix.matrix_(1, 0) << aRotationMatrix.matrix_(1, 1) << aRotationMatrix.matrix_(1, 2);
    ostk::core::utils::Print::Line(anOutputStream)
        << aRotationMatrix.matrix_(2, 0) << aRotationMatrix.matrix_(2, 1) << aRotationMatrix.matrix_(2, 2);
...
RotationMatrix RotationMatrix::RX(const Angle& aRotationAngle)
{
    ...
    matrix << 1.0, 0.0, 0.0, 0.0, +std::cos(rotationAngle_rad), +std::sin(rotationAngle_rad), 0.0,
        -std::sin(rotationAngle_rad), +std::cos(rotationAngle_rad);
...
RotationMatrix RotationMatrix::RY(const Angle& aRotationAngle)
{
    ...
    matrix << +std::cos(rotationAngle_rad), 0.0, -std::sin(rotationAngle_rad), 0.0, 1.0, 0.0,
...
RotationMatrix RotationMatrix::RZ(const Angle& aRotationAngle)
{
    ...
    matrix << +std::cos(rotationAngle_rad), +std::sin(rotationAngle_rad), 0.0, -std::sin(rotationAngle_rad),
        +std::cos(rotationAngle_rad), 0.0, 0.0, 0.0, 1.0;

    return RotationMatrix(matrix);
}

RotationMatrix RotationMatrix::Rows(const Vector3d& aFirstRow, const Vector3d& aSecondRow, const Vector3d& aThirdRow)
{
    ...
    matrix << aFirstRow(0), aFirstRow(1), aFirstRow(2), aSecondRow(0), aSecondRow(1), aSecondRow(2), aThirdRow(0),
        aThirdRow(1), aThirdRow(2);

    return RotationMatrix(matrix);
}

RotationMatrix RotationMatrix::Columns(
...
)
{
    ...
    matrix << aFirstColumn(0), aSecondColumn(0), aThirdColumn(0), aFirstColumn(1), aSecondColumn(1), aThirdColumn(1),
...
RotationMatrix RotationMatrix::Quaternion(const rotation::Quaternion& aQuaternion)
{
    ...
    matrix << m_00, m_01, m_02, m_10, m_11, m_12, m_20, m_21, m_22;
